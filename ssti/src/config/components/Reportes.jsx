...
import { FaRegTrashAlt, FaPencilAlt, FaFileExport, FaAngleLeft, FaAngleRight } from 'react-icons/fa';
...
function Reportes() {
      ...
      fetch('http://localhost:8080/ssti/servicio/')
        ...
        .then(data => {
          if (data.status === 'OK' && !data.error) {
            const filteredData = data.data.filter(item => item.estado === 'Terminado');
  ...
  useEffect(() => {
    const filtered = data.filter(item => {
      ...
      const serie = item.serie ? item.serie.toLowerCase() : "";
      ...
      return (
        ...
        serie.includes(searchTerm.toLowerCase())
      );
    });
          ...
          <tbody>
            {currentItems.map((item, index) => (
              ...
              >
                ...
                <td className="px-6 py-4">{item.serie || "N/A"}</td>
                <td className="px-6 py-4 flex justify-between">
                  <FaPencilAlt className="text-teal-500 cursor-pointer hover:text-teal-700" title="Editar" />
                  <FaFileExport className="text-blue-500 cursor-pointer hover:text-blue-700" title="Exportar" />
